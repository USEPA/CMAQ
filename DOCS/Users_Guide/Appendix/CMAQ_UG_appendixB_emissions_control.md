...
- 'Basis' - Specifies whether the scaling option should apply without consideration of mass/mole conversions, or if the operation should conserve moles or mass. CMAQ includes a lookup table of molecular weights for known emission species (in desid_vars.F) and can use these to translate molar and mass emission rates from the input file to rates in units corresponding to the CMAQ internal species. CMAQ determines the units of the emission species by reading the file header (i.e. it is important the units are accurate. Options for input are:
  ...
  - Example 1: CMAQ gas-phase species X is mapped to particle-phase emisison variable Y (units in g/s) with a scale factor of B. Emissions for X are needed in mol/s.
    - If Basis is set to MASS, then the scale factor B is multiplied by the MW of X to convert to mol/s.
    - If Basis is set to MOLE, then the scale factor B is divided by the MW of Y to convert to mol/s
    - If Basis is set to UNIT, then the scale factor B is not multiplied by either MW. The conversion to mol/s is assumed to be implicit in the provided scale factor.
  - Example 2: CMAQ gas-phase species X is mapped to gas-phase emisison variable Y (units in mol/s) with a scale factor of B. Emissions for X are needed in mol/s.
    - If Basis is set to MASS, then the scale factor B is multiplied by the MW of Y (for g/s) and divided by the MW of X to convert back to mol/s.
    - If Basis is set to MOLE, then the scale factor B is not multiplied by either MW. Mole conservation is assumed to hold.
    - If Basis is set to UNIT, then the scale factor B is not multiplied by either MW. Mole conservation is assumed to hold.
  - Example 3: CMAQ particle-phase species X is mapped to particle-phase emisison variable Y (units in g/s) with a scale factor of B. Emissions for X are needed in g/s.
    - If Basis is set to MASS, then the scale factor B is not multiplied by either MW. Mass conservation is assumed to hold.
    - If Basis is set to MOLE, then the scale factor B is divided by the MW of Y and multiplied by the MW of X to convert back to g/s.
    - If Basis is set to UNIT, then the scale factor B is not multiplied by either MW. Mass conservation is assumed to hold.
