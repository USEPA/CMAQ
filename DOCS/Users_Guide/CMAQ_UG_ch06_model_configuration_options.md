...
As of version CMAQ version 5.4 the windblown dust module no longer accepts additional land use information beyond the land use information contained in the MCIP files. Users are strongly encouraged to enable the windblown dust module only for configurations using WRF version 4+ and the PX LSM. Other LSMs calculate soil properties at depths that may not be consistent with assumptions in the windblown dust module. See [Appendix A](Appendix/CMAQ_UG_appendixA_model_options.md) on further information on how to specify the land use information for the windblown dust module.
