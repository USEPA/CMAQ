...
ICON can also generate a gridded netCDF file of initial conditions representing prespecified spatial patterns for use in transport algorithm testing (*patterns*).  See the documentation for the [transport test patterns](../../PREP/icon/TRANSPORT_TEST_PATTERNS.md) for details on the ICs generated with the *patterns* option.

The configuration options for ICON include choosing whether the initial conditions are generated from an existing CCTM output file, from an ASCII profile, or are based on a prespecified pattern, and defining the horizontal and vertical grids and time for which initial conditions are to be generated. Information on configuring ICON for the different kinds of input data, environment variables, input and output files, compiling and running ICON are provided in the [README.md](../../PREP/icon/README.md) file in the PREP/icon folder.
...
BCON can also generate a netCDF file of boundary conditions representing prespecified spatial patterns for use in transport algorithm testing (*patterns*).  See the documentation for the [transport test patterns](../../PREP/bcon/TRANSPORT_TEST_PATTERNS.md) for details on the ICs generated with the *patterns* option.
...
The configuration options for BCON include choosing whether the boundary conditions are generated from an existing CCTM output file, from an ASCII profile, or are based on a prespecified pattern, and defining the horizontal and vertical grids and time period for which boundary conditions are to be generated. Information on configuring BCON for the different kinds of input data, environment variables, input and output files, compiling and running BCON are provided in the [README.md](../../PREP/bcon/README.md) file in the PREP/bcon folder.

### 4.1.4 Optional Pre-Processor
...
In the WRF-CMAQ two-way coupled model, dust model still works since the equivalent various type of land use fraction in GRIDCRO2D is available from WRF directly.
