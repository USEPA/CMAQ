...
datasets, the BCs generated by BCON can be time varing, time independent, and
...
generated from CONC files will be either time varying or time independent.

There are three distinct modes of operation for BCON.  When running BCON, the
user must specify whether to generate BCs (1) from an existing CONC file
...
patterns](TRANSPORT_TEST_PATTERNS.md) for details on the BCs generated with the
*patterns* option.
...
|BCTYPE|regrid, profile, patterns|Specifies which BC type to generate.|
