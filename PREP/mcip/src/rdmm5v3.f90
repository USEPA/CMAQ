...
!-------------------------------------------------------------------------------
  ...
  LOGICAL :: gotalbedo,  gotglw,     gotgroundt, gothfx,     gotisltyp,   &
             gotlai,     gotlanduse, gotlatcrs,  gotlh,      gotloncrs,   &
             gotmapcrs,  gotmapdot,  gotmol,     gotpp,      gotpsa,      &
             gotq2,      gotqca,     gotqga,     gotqia,     gotqra,      &
...
!-------------------------------------------------------------------------------
  ...
  gotlh      = .FALSE.  ;  gotpp      = .FALSE.  ;  gotpsa     = .FALSE.
  gotqca     = .FALSE.  ;  gotqga     = .FALSE.  ;  gotqia     = .FALSE.
  ...
  findtime: DO
    ...
    ELSE IF ( iflag == 1 ) THEN
      ...
      keeptime: IF ( keep == 1 ) THEN
        ...
        IF ( ( vname == 'LHFLUX   ' ) .AND. ( .NOT. gotlh ) ) THEN
          IF ( ( SIZE(lh,1) == SIZE(data_in,2) ) .AND.  &
               ( SIZE(lh,2) == SIZE(data_in,1) ) ) THEN
            ! latent heat flux [W m^-2]
            DO j = start_index(2), end_index(2)
              DO i = start_index(1), end_index(1)
                lh(j,i) = data_in(i,j,1,1)
              ENDDO
            ENDDO
            gotlh = .TRUE.
            WRITE (*,f6000) 'LH       ', lh(lprt_metx,lprt_mety), TRIM(units)
          ELSE
            WRITE (*,f9400) TRIM(pname), TRIM(var)
            CALL graceful_stop (pname)
          ENDIF
