...
!-------------------------------------------------------------------------------
  ...
  DO n = 1, md3index
    WRITE (*,ifmt1) TRIM(md3vname(n)), (md3(lprt_col,lprt_row,k,n),k=1,nlays)
  ENDDO

  IF ( luvcout > 0 ) THEN  ! Output u- and v-component winds on C grid
    WRITE (*,ifmt1) 'UWINDC',  (uu_s(lprt_col,lprt_row,k),k=1,nlays)
    WRITE (*,ifmt1) 'VWINDC',  (vv_t(lprt_col,lprt_row,k),k=1,nlays)
  ENDIF
