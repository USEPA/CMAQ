...
!-------------------------------------------------------------------------------
  ...
  IF ( first ) THEN
    ...
    IF ( ifcld3d ) THEN
      icld = 1
      idx = mc3index + nqspecies + itke + ipv + iwout + icld
      vtype3d(idx) = m3real
      vname3d(idx) = 'CFRAC_3D'
      units3d(idx) = 'fraction'
      vdesc3d(idx) = '3D resolved cloud fraction'
    ELSE
      icld = 0
    ENDIF
    ...
    ftype3d = grdded3
    nvars3d = mc3index + nqspecies + itke + ipv + iwout + icld
    nlays3d = nlays
  ...
  DO row = 1, nrows
    ...
    DO col = 1, ncols
      ...
      DO lvl = 1, nlays
        ...
        IF ( ifcld3d ) THEN
          cfrac3d_c  (col,row,lvl) = xcfrac3d  (c,r,lvl)
        ENDIF
  ...
  DO r = 1, nthik
    DO c = 1 + nthik, ncols_x
      ...
      DO lvl = 1, nlays
        ...
        IF ( ifcld3d ) THEN
          cfrac3d_b  (idx,lvl) = xcfrac3d  (c,r,lvl)
        ENDIF
...
!-------------------------------------------------------------------------------
  ...
  DO n = 1, mc3index
    WRITE (*,ifmt1) TRIM(mc3vname(n)), (mc3(lprt_col,lprt_row,k,n),k=1,nlays)
  ENDDO

  DO n = 1, nqspecies
    WRITE (*,ifmt1) TRIM(qc3vname(n)), (qc3(lprt_col,lprt_row,k,n),k=1,nlays)
  ENDDO

  IF ( ( iftke ) .AND. ( .NOT. iftkef ) ) THEN  ! TKE on half-layers
    WRITE (*,ifmt1) 'TKE',  (tke_c(lprt_col,lprt_row,k),k=1,nlays)
  ELSE IF ( ( iftke ) .AND. ( iftkef ) ) THEN   ! TKE on full-levels
    WRITE (*,ifmt1) 'TKEF', (tke_c(lprt_col,lprt_row,k),k=1,nlays)
  ENDIF

  IF ( lpv > 0 ) THEN  ! Output potential vorticity
    WRITE (*,ifmt1) 'PV',  (pvc_c(lprt_col,lprt_row,k),k=1,nlays)
  ENDIF

  IF ( lwout > 0 ) THEN  ! Output vertical velocity
    WRITE (*,ifmt1) 'WWIND',  (wwind_c(lprt_col,lprt_row,k),k=1,nlays)
  ENDIF

  IF ( ifcld3d ) THEN  ! 3D resolved cloud fraction
    WRITE (*,ifmt1) 'CFRAC_3D',  (cfrac3d_c(lprt_col,lprt_row,k),k=1,nlays)
  ENDIF
