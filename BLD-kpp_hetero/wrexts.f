...
!      INTEGER,          INTENT( IN ) :: IRR( MAXRXNUM,MAXPRODS+3 )
!      REAL,             INTENT( IN ) :: SC ( MAXRXNUM,MAXPRODS )
...
1043  FORMAT( /'C', 1X, 'The following are reserved symbols declared in this',
     ...
     &        /'C', 4X, 'CHEMISTRY_SPC  = Names of species in chemical mechanism'
     &        /'C', 4X, 'CGRID_INDEX    = CGRID Index of species in chemical mechanism'
     &        /'C', 4X, 'SPECIES_TYPE   = Group or type of species in chemical mechanism'
...
1049  FORMAT( /'C', 4X, 'MXPRD     = Maximum number of mechanism reaction',
     ...
     &        /'C', 4X, 'MHETERO   = Number of mechanism heteorogenous reactions',
     &        /'C', 4X, 'NHETERO   = Number of unique heteorogenous rate constants', 
     &        /'C', 4X, 'IHETERO   = Reactions list pointer to heteorogenous',
     &              1X, 'reactions and tables' )
...
c-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-
      ...
      IF ( MHETERO .NE. 0 ) THEN
         ...
         WRITE( EXUNIT_RXCM, 1721 ) MHETERO
1721     FORMAT( /6X, 'INTEGER, PARAMETER', 1X, ':: MHETERO =', I4 )

         WRITE( EXUNIT_RXCM, 1723 )
1723     FORMAT(  6X, 'INTEGER', 12X, ':: IHETERO( MHETERO,2 )' )

         WRITE( EXUNIT_RXDT, 1725 ) '1'
1725     FORMAT( /6X, 'DATA ( IHETERO( IRXXN,', A, ' ), IRXXN = 1, MHETERO ) /' )

         CALL WRBF6 ( EXUNIT_RXDT, 10, MHETERO, IHETERO( 1,1 ) )

         WRITE( EXUNIT_RXDT, 1725 ) '2'

         CALL WRBF6 ( EXUNIT_RXDT, 10, MHETERO, IHETERO( 1,2 ) )

 
      ELSE

         WRITE( EXUNIT_RXCM, 1727 )
1727     FORMAT( /'C Heteorogeneous reactions information not available ...'
     &           /6X, 'INTEGER, PARAMETER', 1X, ':: MHETERO = 0' )

         WRITE( EXUNIT_RXCM, 1729 )
...
1765     FORMAT( /6X, 'DATA ( HETERO( IRXXN ), IRXXN = 1, NHETERO ) /' )
         ...
         IFLD1 = 1
         DO IFLD0 = 1, NHETERO / 3
            IFLD2 = IFLD1 + 2
            WRITE( EXUNIT_RXDT, 1769 ) ( BUFF20( IRX ), IRX = IFLD1, IFLD2 )
1769        FORMAT(5X, '&', 2X, 4A20 )
            IFLD1 = IFLD2 + 1
         END DO
         IF ( IFLD1 .LE. NHETERO )
     &      WRITE( EXUNIT_RXDT, 1769 ) ( BUFF20( IRX ), IRX = IFLD1, NHETERO )

      ELSE

         WRITE( EXUNIT_RXCM, 1757 )
1757     FORMAT( /'C Heteorogeneous reaction information not available ...'
     &           /6X, 'INTEGER, PARAMETER', 1X, ':: NHETERO = 0' )
                                                
         WRITE( EXUNIT_RXCM, 1758 )
1758     FORMAT( /'C Heteorogeneous reaction information not available ...'
     &           /6X, 'CHARACTER( 16 )', 4X, ':: HETERO( 1 )' )


      END IF

      IF ( NHETERO .EQ. 0 .AND. NPHOTAB .NE. 0 ) THEN
...
1807     FORMAT( /5X, ' COMMON     / MECHRX3 /'
     ...
     &           /5X, '&             RXLABEL' )
      ...
      ELSE IF ( NHETERO .EQ. 0 .AND. NPHOTAB .EQ. 0 ) THEN
...
1809     FORMAT( /5X, ' COMMON     / MECHRX3 /'
     &           /5X, '&             RXLABEL' )
      ...
      ELSE IF ( NHETERO .NE. 0 .AND. NPHOTAB .NE. 0 ) THEN

         WRITE( EXUNIT_RXCM, 1857 )
1857     FORMAT( /5X, ' COMMON     / MECHRX3 /'
     &           /5X, '&             PHOTAB,',
     &           /5X, '&             HETERO,',
     &           /5X, '&             RXLABEL' )
      
      ELSE IF ( NHETERO .NE. 0 .AND. NPHOTAB .EQ. 0 ) THEN

         WRITE( EXUNIT_RXCM, 1858 )
1858     FORMAT( /5X, ' COMMON     / MECHRX3 /'
     &           /5X, '&             HETERO,',
     &           /5X, '&             RXLABEL' )
