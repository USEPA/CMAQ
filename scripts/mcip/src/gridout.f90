...
! $Header: /project/work/rep/MCIP2/src/mcip2/gridout.F,v 1.6 2007/08/03 20:49:45 tlotte Exp $ 
...
!-------------------------------------------------------------------------------
  ...
  ELSE IF ( met_urban_phys >= 1 ) THEN
    nvars3d = gb2index + 1             ! include PURB
  ELSE
    nvars3d = gb2index
  ENDIF
...
!-------------------------------------------------------------------------------
  ...
  IF ( iflufrc ) THEN
    WRITE (*,6000) TRIM(vname3d(gc2index+1)),  &
                   gc2(lprt_col,lprt_row,gc2index+1), units3d(gc2index+1)
    DO ilu = 1, nummetlu
      WRITE (*,6000) TRIM(vname3d(gc2index+1+ilu)),          &
