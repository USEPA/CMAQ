...
! $Header: /project/work/rep/MCIP2/src/mcip2/metcro.F,v 1.8 2007/08/03 20:51:37 tlotte Exp $ 
...
!-------------------------------------------------------------------------------
  ...
  IF ( first ) THEN
    ...
    IF ( lddep > 0 ) THEN
      ...
      DO ilg = 1, ltotg
        idx = mc2index + iwr + isoil + isat + ilg
        vtype3d(idx) = m3real
        spcname      = TRIM(xdepspc(ilg))
        vname3d(idx) = 'VD_' // spcname(1:13)
        units3d(idx) = 'M/S'
    ...
    ftype3d = grdded3
    nvars3d = mc2index + iwr + isoil + isat + ltotg
    nlays3d = 1
    ncols3d = ncols
    nrows3d = nrows
    nthik3d = nthik
...
!-------------------------------------------------------------------------------
  ...
  IF ( ifwr ) THEN
    idx = mc2index + iwr
    WRITE (*,6000) TRIM(vname3d(idx)), wr_c(lprt_col,lprt_row), units3d(idx)
  ENDIF

  IF ( ifsoil ) THEN
    idx = mc2index + iwr + 1
    WRITE (*,6000) TRIM(vname3d(idx)), soim1_c(lprt_col,lprt_row), units3d(idx)
    idx = mc2index + iwr + 2
    WRITE (*,6000) TRIM(vname3d(idx)), soim2_c(lprt_col,lprt_row), units3d(idx)
    idx = mc2index + iwr + 3
    WRITE (*,6000) TRIM(vname3d(idx)), soit1_c(lprt_col,lprt_row), units3d(idx)
    idx = mc2index + iwr + 4
    WRITE (*,6000) TRIM(vname3d(idx)), soit2_c(lprt_col,lprt_row), units3d(idx)
    idx = mc2index + iwr + 5
    WRITE (*,6000) TRIM(vname3d(idx)), sltyp_c(lprt_col,lprt_row), units3d(idx)
  ENDIF

  IF ( lsat == 1 ) THEN
    idx = mc2index + iwr + isoil + isat
    WRITE (*,6000) TRIM(vname3d(idx)), cldtr_c(lprt_col,lprt_row), units3d(idx)
  ENDIF
