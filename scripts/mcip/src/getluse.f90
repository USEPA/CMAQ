...
!-------------------------------------------------------------------------------
  ...
  ELSE IF ( ( met_lu_src(1:3) == "NLC" ) .AND. ( nummetlu == 50 ) ) THEN
    xlusrc = "NLCD50"
    lwater = 1  ! and also 31 and 48...to be combined, below
  ...
  ELSE IF ( ( met_lu_src(1:3) == "NLC" ) .AND. ( nummetlu == 40 ) ) THEN
    xlusrc = "NLCD40"
    lwater = 17 ! and also 21...to be combined, below
    lice   = 15 ! and also 22...to be combined, below
    DO i = 1, nummetlu
...
!-------------------------------------------------------------------------------
  ...
  ELSE
    ...
    WHERE ( NINT(xdluse) == lwater )  ! water
      xlwmask = 0.0
    ELSEWHERE  ! land
      xlwmask = 1.0
    END WHERE
